 ...
 * raised per reason. These are events dispatched, not messages delivered: the weekly
 * dedup key decides delivery downstream, so a run can report work while sending
 * nothing, which is the intended steady state on a case that stays blocked.
 */
final readonly class BlockedCaseAlertReport
{
    ...
    {
        return $this->stampDutyDue + $this->regularizationNoticeDateMissing;
    }
