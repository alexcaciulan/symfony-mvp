...
REASON="Modificări necomise detectate în paths review-abile (src/, tests/, config/, migrations/, assets/, templates/, translations/, docker/, data/ sau fișiere root infra) cu diff hash $DIFF_HASH, încă nerevizuite. Înainte de a încheia pasul, invocă subagent_type='lexrecovery-code-reviewer' — calitate cod + security (PHP/Symfony, teste, OWASP, Doctrine, infra). NOTĂ: agentul lexrecovery-legal-reviewer este DEZACTIVAT temporar din hook — NU îl invoca automat aici. După ce primești raportul, prezintă-l user-ului (verdict + blockers), apoi scrie hash-ul $DIFF_HASH în .claude/.last-review-hash pentru debounce. Dacă agentul găsește BLOCKERS, repară-le ÎNAINTE de a cere confirmare pentru git commit. Algoritm stamp identic cu hook-ul (vezi agent definitions §6)."
