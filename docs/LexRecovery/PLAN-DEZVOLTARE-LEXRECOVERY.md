...
| 3.0 | Wizard | Step 0 wizard "Documente sursă": upload + procesare async (Mercure-first cu polling fallback) + side-card agregat „Date detectate" + `PrefillFromExtractionService` (mutat din 3.1 per analiză 2026-05-11) + migrare `Document.legalCase` NULLABLE | 1.5z | 2.5.8, 2.6, 2.7 | 100% | ✅ DONE 2026-05-11 (`8a54667` base + `7630abc` full creditor/debtor coverage + `012293d` coverage-weighted globalConfidence + dev knobs) — cascadă funcțională end-to-end pe documente reale; **REVIZIE retroactivă Pas 2.5.2** formula globalConfidence migrată la coverage-weighted (`CoverageConfidenceCalculator`) |
| 3.1 | Wizard | DTOs + Forms 4 pași (1-4: Creditor, Debitor, Creanță, Confirmare) cu validări complete + meta `autoFilled` (NU `_autoFilled`) + indicator vizual câmp auto vs manual. Notă: DTOs scheletice + `PrefillFromExtractionService` deja create în Pas 3.0 — aici doar refactor cu validări | 1z | 1.1, 2.1-2.3, 3.0 | 100% | ✅ DONE 2026-05-11 (necomis — code review pending) — 5 DTOs cu validări complete (`Step{1Creditor,2DebtorEntry,2DebtorsData,3Claim,4Confirmation}Data`) + 6 FormTypes în `src/Form/Wizard/` + `AutoFilledMarker::apply()` reutilizat în `finishView()` pentru `data-auto-filled` + `ValidCui` constraint reutilizează `PiiMasker::isValidCui()` + refactor `PrefillFromExtractionService` cu garantare min 1 debtor + ~63 teste noi (DTO/Form/Validator/Prefill extins) + i18n RO/EN `wizard.step{1-4}.*` + `validators.*.wizard.*`. `PrefillFromExtractionService` mutat la 3.0 per D2; Step3 `relationshipType` doar COMERCIAL (CIVIL hidden per C4) |
| 3.2 | Wizard | `CaseWizardController` extins step 1-4 + session storage + templates + **integrare obligatorie `OpAdmissibilityValidator` la Step 4 submit** (ERROR blochează, WARNING cu confirmare bifabilă) + audit `wizard_submit` cu `fields_auto[]`/`fields_manual[]` + reuse Creditor existent pe UNIQUE(user, cui) | 1z | 3.1 | 100% | ✅ DONE 2026-05-12 (`8acc7f7`) — 4 rute noi (creditor/debtor/claim/confirmation) + session bag DTOs + transactional persist (LegalCase + Debtors + Document.legalCase bulk + AuditLog WIZARD_SUBMIT) + reuse Creditor C3 + Step3 sidebar calcule server-rendered + Step4 4 cards summary cu ERROR/WARNING display + acknowledgedWarnings condițional via validation_groups + GDPR notice + 10 teste WebTestCase verzi; legal LEGAL-CLEAN cu DOC-FIX (eliminat plafon fals 10k RON), code COMMIT-READY după 2 BLOCKERS + 2 W fix-uite |
| 3.3 | Wizard | `Step3ClaimLiveComponent` + **`Step2DebtorsLiveComponent`** (Live Component pentru multi-debtor, decis în loc de CollectionType vanilla) + UX Autocomplete creditor (cu `CreditorRepository::createAutocompleteQueryBuilder` scoped per-user) + `debitor-anaf-lookup_controller.js` + `LookupController` `/api/anaf-lookup/{cui}` | 1z | 3.2, 2.1-2.4 | 20% | ✅ DONE 2026-05-12 (`b778665`) — primul Live Component din codebase; `LiveCollectionTrait` pentru Step2 multi-debtor (mutează `formValues` NU DTO); `Step3ClaimLiveComponent` re-calculează server-side dobândă+taxă fără AJAX manual; UX Autocomplete creditor scoped per-user; ANAF lookup `/api/anaf-lookup/{cui}` cu regex requirement + PiiMasker checksum + rate limiter 10/h; cleanup legacy (`CompanyLookupTest` + controller-ele moarte → -3 erori baseline); 21 teste noi → suite 708/39/1 (vs baseline 673/40/4: +35 teste, -1 error, -3 failures) |
| 4.1 | Termene | `DeadlineService` + `WorkingDayResolver` (4 metode create + Paște ortodox Meeus + 10 sărbători RO + override YAML) | 0.5z | 1.1 | 0% | ✅ DONE 2026-05-15 (`3d9bef9`) — 29 teste verzi; W1 legal `paymentNoticeDate` semantics deferred la Pas 4.2 |
| 4.2 | Termene | `DeadlineCreationSubscriber` cu 3 listeners (postPersist + 2 workflow events) + idempotency repository | 0.5z | 4.1, 1.3 | 80% | ✅ DONE 2026-05-15 (`dde49eb`) — 9 teste verzi; B1 legal addresat prin disclaimer pe LegalDeadline.description; W3 legal — drop prorogare PRESCRIPTIE (drept material NCC, NU procedural CPC) |
| 4.3 | Termene | UI tab Termene wire-up: mark complete (Turbo Stream + optimistic UI) + add custom JUDECATA + rulingCommunicationDate input (closes W1 legal Pas 4.2) | 0.5z | 4.1 | 0% | ✅ DONE 2026-05-15 (`52ad9fc`) — 14 teste verzi; Faza 4 COMPLETĂ end-to-end |
...
| 5.2 | PDF | `PaymentOrderRequestGeneratorService` + opis + `CaseFilesPackager` + ZIP download + workflow `depune_cerere` | 1z | 5.1 | 50% | ✅ DONE 2026-05-16 (`af6e247`) — Faza 5 COMPLETĂ; 27 teste verzi; B1 legal taxa timbru RON + W1 multi-debitor + Opțiunea A enum isAutoGenerated + 1-query optim |
...
- **Comentarii în cod: engleză, scurte, fără referințe la plan/pași.** Comentariile din cod (docblock-uri, inline) se scriu **în engleză** și doar acolo unde aduc valoare (explică „de ce", nu „ce" face codul evident). **NU se pun referințe la planul de dezvoltare sau la pași** în cod, pe clase/metode/proprietăți (ex: `// Pas 6.2 — ...`, `// revizia C5`, `// Redenumire din MonitorCourtCasesCommand`, `@note livrat la Pas X.Y`). Acel istoric aparține planului și fișierelor de memorie, nu codului sursă. Comentariul descrie codul așa cum e acum, independent de când/cum a fost construit. Stabilit 2026-05-24 după ce comenzile din Pas 6.2 au ajuns pline de adnotări „Pas 6.2 — ...".
- **Stringuri CLI/cron: engleză literal, NU în română, NU prin translator.** Output-ul comenzilor de consolă (`$io->title()`, `$io->section()`, linii success/error), descrierile de la `addOption()`/`addArgument()` și atributul `description:` al comenzii se scriu **în engleză literal** — CLI e o suprafață pentru operator/cron, ca log-urile (care deja rămân EN). Exemple: `$io->title('Verificare termene ...')` → `$io->title('Deadline check ...')`; `addOption(..., 'Dispecerizează un singur dosar după ID')` → `addOption(..., 'Dispatch a single case by ID')`. Decizie user 2026-05-24 (varianta „engleză literal" peste „prin translator"). Textele user-facing (avocat în UI) rămân în română prin translator (regula de mai sus).
...
**Rezultat**: ✅ DONE 2026-05-11 — 3 commit-uri (`8a54667` base + `7630abc` full creditor/debtor coverage + Anthropic API shape fix + `012293d` coverage-weighted globalConfidence). Livrat: **migrare `Document.legalCase` NULLABLE** + fallback path `_pending/{userId}/{uuid}/` pentru documente pre-creare-dosar; **5 rute wizard step 0** (`case_wizard_step0_*` — show, upload, status, delete) în `CaseWizardController`; **listener Mercure metadata-only** + **polling fallback** + **Turbo Stream** pentru update side-card; **`PrefillFromExtractionService`** (mutat din Pas 3.1 per D2 — agregare cross-document best-confidence ≥0.8, threshold 0.6, fallback DTOs structural valide pe empty input); **`CoverageConfidenceCalculator` nou** — formula `globalConfidence` migrată de la `mean` (Pas 2.5.2) la coverage-weighted `sum/25` cu **REVIZIE retroactivă Pas 2.5.2** documentată. **11 bug-uri descoperite live pe documente reale** (Anthropic API `system` field shape, AiVision 5MB→3.7MB base64 cap, section role compound header attribution, IBAN regex, OcrText.supports() PDF text-layer skip etc) — fix-uite. **39 teste noi** (15 base + 9 extension + 15 follow-up); 586/586 cu 41/4 IDENTIC baseline Pas 2.7 (zero regresii). **Dev knobs** `EXTRACTION_FORCE_STRATEGY` + `SKIP_STRATEGIES` pentru testare cascadă pe staging. 2 code-reviewer NEEDS-FIX fix-uite înainte de commit.
...
**Rezultat**: ✅ DONE 2026-05-11 (necomis — code review pending). Livrat: **5 DTOs** refactorizate cu validări Symfony (`Step1CreditorData` cu `Expression` xor creditorId/manual + `Step2DebtorEntry` cu ANAF+BPI metadata + `Step2DebtorsData` min 1 max 5 + `Step3ClaimData` cu currency RON/EUR și COMERCIAL-only + `Step4ConfirmationData` cu 3 IsTrue checkboxes). **6 FormTypes** în `src/Form/Wizard/` (`Step{1Creditor,2DebtorEntry,2Debtors,3Claim,4Confirmation}Type` + `AutoFilledMarker::apply()` util reutilizat în `finishView()` cu garda `isset()`). **Validator CUI custom** (`src/Validator/Constraints/ValidCui` + `ValidCuiValidator`) reutilizează `PiiMasker::isValidCui()` (DRY cu PdfParser extraction). `Step3ClaimType` dropdown `relationshipType` doar `COMERCIAL` (CIVIL ascuns — C4); `legalGround` filtrat prin `LegalGroundCategory::isOpEligible()`. `PrefillFromExtractionService` refactor: `aggregateForDebtors()` garantează min 1 entry chiar pentru `documentIds=[]`; DTOs returnate structural valide. **~63 teste noi** (5×DTO ~29 + 5×FormType ~25 + ValidCui 8 + Prefill extins 13). **i18n complet** RO+EN: `wizard.step{1-4}.*` (labels + placeholders) + `validators.*.wizard.step*.error.*`. **Notă scope**: `PrefillFromExtractionService` mutat la Pas 3.0 per D2 (revizie 2026-05-11) — pas 3.1 = refactor + finalizare + Forms + tests.
...
**Rezultat**: ✅ DONE 2026-05-12 (`8acc7f7`). Livrat: **4 rute noi** în `CaseWizardController` (creditor/debtor/claim/confirmation cu GET+POST handlers) extinse de la skeletonul Pas 3.0; **session bag** extinsă cu DTOs `Step1CreditorData` / `Step2DebtorsData` / `Step3ClaimData` (serializare PHP nativă, plain classes); **Step 4 GET** construiește `LegalCase` skeleton din session → rulează `OpAdmissibilityValidator::validate()` → split ERROR/WARNING → re-rulează `InterestCalculatorService` + `StampDutyCalculator` + `CompetentCourtResolver` pentru sidebar dark navy; **Step 4 POST** tranzacțional cu `EntityManager::wrapInTransaction`: rate limiter `case_creation` (10/h) → reuse Creditor pe UNIQUE(user, cui) (C3) → persist LegalCase cu `calculatedInterest`/`stampDuty`/`court` (nullable per C5) → cascade Debtor + FK → `DocumentRepository::attachToCase()` bulk DQL UPDATE → `AuditLogService::log('wizard_submit', ..., CATEGORY_WIZARD_SUBMIT)` cu `newData = {fields_auto, fields_manual, extractedDocIds, admissibility_warnings}` → flash success → redirect `dashboard_cases`. **`Step4ConfirmationData`** refactor: `IsTrue` pe `acknowledgedWarnings` mutat în grup `with_warnings`; controllerul activează grupul când validator emite WARNING-uri (`validation_groups: ['Default', 'with_warnings']`). **`AuditLogService::CATEGORY_WIZARD_SUBMIT`** constant nouă (paritate cu CATEGORY_AI_EXTRACTION). **5 template-uri noi** (`_step{1-4}_*_content.html.twig` + `_wizard_field_badge.html.twig`): step 1/2/3 forms (Step 2 cu non-JS add-debtor handler `_action=add_debtor` — LiveComponent vine în Pas 3.3); step 3 sidebar dark navy `from-lex-navy to-lex-navy-dark` cu 3 cards calcule + BNR breakdown accordion; step 4 cu 4 summary cards + per-field icon ⚡/✎ badges + GDPR notice amber + ERROR alert roșu (submit disabled) sau WARNING amber + acknowledgedWarnings checkbox + CTA verde gradient. **i18n RO+EN** ~120 chei noi: `wizard.step{1-4}.{title,subtitle,help,sidebar,alert,card,gdpr,flash,final_calculations,button,badge,error}` + `wizard.button.{back,continue}`; `validators.{ro,en}.yaml` extins cu `wizard.step4.error.must_acknowledge_warnings`. **DOC-FIX retroactiv** (legal review): șterse cheile moștenite `wizard.step4.amount_hint`/`amount_max` cu plafon fals "10.000 RON OUG 80/2013" — OP NU are plafon valoric superior, era confuzie cu procedura cererii cu valoare redusă (CPC art. 1025). **10 teste integration** noi (`CaseWizardControllerStep1To4Test` WebTestCase): GET empty session → 200 cu "Date creditor"; POST step 1 → 302 spre debtor; GET /confirmation fără pași anteriori → 302 spre primul lipsă; ERROR ANAF radiat → alert roșu + submit disabled; WARNING PF → acknowledgedWarnings checkbox cerut → POST fără bifare → HTTP 422 (Symfony 7 default pentru form invalid); happy path POST → LegalCase + Debtor + AuditLog WIZARD_SUBMIT persistate + redirect `/dashboard/cases`; reuse Creditor pe UNIQUE(user, cui) → un singur Creditor în DB; insolvency_not_verified → ERROR; bulk attach Document.legalCase. Suite full **673 tests** cu 40 errors / 4 failures (baseline Pas 3.1: 586/41/4 — zero regresii, +87 tests verzi, -1 error datorat drop placeholder 501 test). **Code review** COMMIT-READY după 2 BLOCKERS (`must_acknowledge_warnings` în domeniu `validators` + `strict_types` în `DocumentRepository`) + 2 W (FormInterface/TranslatorInterface imports + flash creditor_reused refactor în afara tranzacției — out-param `wasReused` din `reuseOrCreateCreditor`). **Legal review** LEGAL-CLEAN cu W (0 BLOCKERS); 3 NIT-uri pentru backlog (calcule re-execute la POST defense in depth, GDPR notice extins în Privacy Policy, debifare inInsolvency post-reverificare BPI). 1 problemă scoasă din scope: rate limiter test 11 submituri (cere setup `no_limit` override conditional, pas 3.3 sau backlog). **Out-of-scope** (Pas 3.3): `Step2DebtorsLiveComponent` + `Step3ClaimLiveComponent`, UX Autocomplete creditor, `debitor-anaf-lookup_controller.js` + endpoint `/api/anaf-lookup/{cui}`.
...
**Rezultat**: ✅ DONE 2026-05-12 (commit `b778665`). Livrat: **primul Live Component din codebase** — `Step3ClaimLiveComponent` (4 LiveProps writable: amount/dueDate ISO/relationshipType/currency; `getInterest()` defensive try/catch DomainException/InvalidArgumentException/RuntimeException → null; NU rezolvă instanța — rămâne la Step 4 cu locality structurat) + `Step2DebtorsLiveComponent` (folosește `LiveCollectionTrait` bundle-provided cu `addCollectionItem`/`removeCollectionItem` care mutează `formValues` LiveProp; max 5 entries enforced vizual via `disabled`; **NU custom LiveActions** pentru că `submitForm()` resetează din POST gol în test, `resetForm()` re-extrage din formView re-hidratat din props nemodificate). **`LookupController`** `/api/anaf-lookup/{cui}` cu `requirements: ['cui' => '(RO)?\d{2,10}']` + `PiiMasker::isValidCui()` checksum validation + consumă rate limiter `company_lookup` (10/h existent) + răspuns JSON `{companyName, cui, address, anafStatus, anafCheckedAt}` la success / `{error: 'exception.anaf.*'}` la 404/429/503; `config/packages/security.yaml` adăugat `path: '^/api', roles: IS_AUTHENTICATED_FULLY`. **`CreditorAutocompleteType`** wraps `BaseEntityAutocompleteType` cu `query_builder` callable Security-aware → `CreditorRepository::createAutocompleteQueryBuilder(User): QueryBuilder` filtrează `WHERE c.user = :user ORDER BY c.name ASC`. **`Step1CreditorType`** extins cu `creditorEntity` (`mapped: false`) + POST_SUBMIT listener bridge `$picked->getId()` → `$dto->creditorId` int → **DTO contract intact** (`reuseOrCreateCreditor()` neschimbat). **2 Stimulus controllers noi**: `debtor-anaf-lookup_controller.js` (blur pe CUI input cu regex client-side `/^(RO)?\d{2,10}$/` + fetch JSON + populează `name`/`address`/`anafStatus`/`anafCheckedAt` + badge emerald „ANAF verificat" + dispatch `toast:show` pe eroare) + `creditor-mode_controller.js` (toggle hide/show pe manualFields card când autocomplete are valoare). **Cleanup legacy**: șterse `assets/controllers/company_lookup_controller.js` + `templates/case/_defendant_fields.html.twig` + `tests/Controller/CompanyLookupTest.php` (4 teste lovind `/case/company-lookup/{cui}` care nu mai existau în routes — failing silent în baseline 673/40/4). **21 teste noi**: 7 `LookupControllerTest` (happy path, RO prefix, route requirement rejection too short/non-numeric, checksum rejection, ANAF unavailable, auth required) + 3 `CreditorRepositoryTest` (scoping per-user, empty result, ordering ASC) + 6 `Step3ClaimLiveComponentTest` (prop change recomputes interest, future dueDate→null, EUR currency→null, CIVIL→null, stamp duty fixed 200 RON, missing amount→null) + 5 `Step2DebtorsLiveComponentTest` (addCollectionItem append, preserve existing entries, removeCollectionItem drops right entry, initial render uses initialFormData, add button disabled at cap). Suite full **708/39/1** (vs baseline Pas 3.2: 673/40/4 → **+35 teste, -1 error, -3 failures** zero regresii). **Decizii confirmate user 2026-05-12**: (1) ANAF endpoint literal `/api/anaf-lookup/{cui}` în `Api/LookupController` (NU reuse legacy `/case/company-lookup/{cui}`); (2) drop fallback non-JS pentru add-debtor (Live Component sursa unică); (3) skip court resolver în Step 3 live — `county`/`locality` rămân omise din LiveProps, sidebar afișează „Instanța se calculează la confirmare", resolver rulează la Step 4 cu LegalCase complet. **Lessons learned**: `LiveCollectionTrait > custom LiveActions` (mută `formValues` NU `initialFormData` DTO — fix de 30 linii eliminat → 5 teste pass instant); CSRF + KernelTestCase necesită push `Request` + `MockArraySessionStorage` în `RequestStack` la setUp pentru `Step2DebtorsType` cu `csrf_protection: true` (sesiunea NU e stateless_token_ids); `path()` validează route requirements stricte — pattern `path('route', { cui: '0000000001' })|replace({ '0000000001': '__CUI__' })` păcălește URL generator-ul cu valoare validă apoi string-replace pentru placeholder JS. **Out-of-scope (post-3.3)**: skeleton loading pe câmpuri pre-populate (rar în practică); polling fallback EventSource (doar dacă staging arată pică); PF/PJ conditional fields (moștenit wizard alignment v2 — afișează ambele seturi în paralel; backlog 3.x); `save-to-library` backend creditor/debtor (UI cosmetic exists; backend la Pas 6.x); rate limiter test cu 11+ cereri pe `/api/anaf-lookup` (necesită `no_limit` override condițional — backlog moștenit Pas 3.2); inline form errors pe Step 2 după POST invalid (Live Component re-mount din `initialFormData` păstrează datele dar nu propagă validation errors din controller submit — backlog UX). Detalii: `~/.claude/projects/-Users-alexc-Downloads-myprojects-symfony-mvp/memory/project_lexrecovery_pas_3_3.md`.
...
### PASUL 4.1 | `DeadlineService` | 0.5 zi | 0% reutilizare ✅ DONE 2026-05-15 (`3d9bef9`)

**Rezultat**: ✅ DONE 2026-05-15 (commit `3d9bef9`). Livrat: `App\Service\Deadline\WorkingDayResolver` (sărbători legale RO complete — 10 fixe per Codul Muncii art. 139 inclusiv 24 ian Legea 147/2012 + 1 iun Legea 114/2022 + Paște ortodox via algoritm Meeus + 13 zile conversie Iulian→Gregorian + Vinerea Mare + Paște luni + Rusalii duminică/luni + override YAML `lexrecovery.working_days.additional_holidays` + guard `\DomainException` pentru ani 2100+) + `App\Service\Deadline\DeadlineService` (4 metode publice — `createPaymentNoticeDeadline` 15 zile CPC art. 1015, `createAppealDeadline` 10 zile CPC art. 1024 cu `$rulingCommunicationDate` explicit, `createPrescriptionDeadline` 3 ani NCC art. 2517 + InvalidArgumentException pe null dueDate, `createHearingDeadline` data dată cu descriere opțională, `markCompleted` idempotent cu User). Entity changes + migrare `Version20260515130649`: `LegalCase.rulingCommunicationDate` DATE_IMMUTABLE nullable + `LegalDeadline.completedBy` ManyToOne User nullable ON DELETE SET NULL. `DeadlineType::defaultPriority()` (rename din `defaultPrioritate()` per `feedback_no_romanian_in_code`) + RASPUNS_SOMATIE MEDIUM→HIGH + JUDECATA HIGH→MEDIUM (aliniere ANALIZA-FLUXURI section 8). `AuditLogService::CATEGORY_DEADLINE_CREATED` + `CATEGORY_DEADLINE_COMPLETED` + `declare(strict_types=1)`. Audit log conține `{deadlineId, type, caseNumber, baseDate, rawDeadline, deadlineDate, prorogated}` pentru reconstrucție timeline. Cleanup oportunist: `SeedDemoCasesCommand` drift `CERERE_IN_ANULARE` HIGH→`defaultPriority()` (= CRITICAL) + drop `'priority'` hardcoded din 3 entries match. 29 teste verzi (13 WorkingDayResolver pure TestCase: weekend skip + sărbători fixe + Easter chain 2026 + Easter 2025 cross-year + Rusalii + additional_holidays config + guard 2100; 14 DeadlineService KernelTestCase: toate 4 metode + idempotent markCompleted + audit log payload prorogated:true + multiple deadlines coexist; 2 entity test extensions). Suita globală **801/29/2** (vs baseline 771/29/2 = +30 teste, zero regresii). Reviews legal LEGAL-CLEAN + code COMMIT-READY după 2 fix-uri (W1 SeedDemoCasesCommand drift + W2 Meeus guard). **Deferred la Pas 4.2**: W1 legal — decizia `paymentNoticeDate` semantici (data emiterii vs data comunicării; serviciul primește data ca parametru, decizia care dată e contextuală la subscriber). Detalii: `~/.claude/projects/-Users-alexc-Downloads-myprojects-symfony-mvp/memory/project_lexrecovery_pas_4_1.md`.
...
### PASUL 4.2 | `DeadlineCreationSubscriber` + `CaseWorkflowSubscriber` | 0.5 zi | 80% reutilizare ✅ DONE 2026-05-15 (`dde49eb`)

**Rezultat**: ✅ DONE 2026-05-15 (commit `dde49eb`). Livrat: `App\EventSubscriber\DeadlineCreationSubscriber` (final class cu 3 listeners — `#[AsDoctrineListener(postPersist)]` pentru PRESCRIPTIE la creare dosar + `#[AsEventListener('workflow.legal_case.entered.SOMATIE_TRIMISA')]` pentru RASPUNS_SOMATIE + `#[AsEventListener('workflow.legal_case.entered.ORDONANTA_EMISA')]` pentru CERERE_IN_ANULARE). Idempotency prin `LegalDeadlineRepository::findOneByCaseAndType()` nouă + guard `$case->getId() === null` pentru fixtures unmanaged. Defensive try/catch + log levels diferențiate (error pentru paymentNoticeDate null = bug ordering controller; info pentru rulingCommunicationDate null = comportament normal — UI viitor va popula). **Deviation de la PLAN aprobat (D2)**: CERERE_IN_ANULARE mutată de la `Doctrine postUpdate` (planul aprobat) la `workflow.entered.ORDONANTA_EMISA` din motive tehnice — flush-in-flush în postUpdate fragil. Comportament UX echivalent. **B1 legal addresat**: disclaimer obligatoriu pe `LegalDeadline.description` pentru RASPUNS_SOMATIE — *„Termen estimativ. Calculat de la data generării somației. Actualizați după confirmarea primirii de către debitor (CPC art. 1015 alin. 1 — termenul curge de la primire)."* — vizibil pe card termen (tab Termene Pas 4.0.5 randează `description`). **W3 legal addresat**: `DeadlineService::createPrescriptionDeadline()` NU mai aplică `nextWorkingDay()` — prescripția e drept material NCC art. 2539-2541, NU termen procedural CPC art. 181; prorogarea ar produce dată mai târzie decât realitatea legală. **W4 legal addresat**: log level error (NU warning) pentru paymentNoticeDate null = eroare programare. **3 tearDown-uri extinse** cu DELETE FROM legal_deadline (CaseSummonsController + CaseWizard) — fix regresie test isolation. `testMultipleDeadlinesCoexistOnSameCase` ajustat (drop manual createPrescriptionDeadline call — subscriber-ul îl creează automat). 9 teste verzi (postPersist happy + skip null dueDate; somatie happy CU disclaimer assert + skip null paymentNoticeDate; ordonanta happy + skip null rulingCommunicationDate; idempotency dual fire via container.get; full cascade 3 deadlines; AMIABIL default status). Suite globală **810/29/2** vs baseline pre-Pas 801/29/2 = **+9 teste, zero regresii**. Schema unchanged (zero migrări noi). Reviews legal LEGAL-NEEDS-FIX (1 BLOCKER + 4 W) + code NEEDS-FIX (0 BLOCKERS + 6 W) → all addresate în acest commit. **Deferred**: W1 legal CERERE_IN_ANULARE recovery UI (Pas 4.3); W2 legal PRESCRIPTIE 10 ani art. 2518 NCC (post-MVP, scope B2B). Detalii: `~/.claude/projects/-Users-alexc-Downloads-myprojects-symfony-mvp/memory/project_lexrecovery_pas_4_2.md`.
...
### PASUL 4.3 | UI tab "Termene" în view dosar (cu optimistic UI) | 0.5 zi | 0% reutilizare ✅ DONE 2026-05-15 (`52ad9fc`)

**Rezultat**: ✅ DONE 2026-05-15 (commit `52ad9fc`). Livrat: `App\Controller\Case\CaseDeadlineController` cu 3 rute POST (`case_deadline_complete /case/{caseId}/deadline/{deadlineId}/complete` cu CSRF + voter + idempotent `DeadlineService::markCompleted()` + Turbo Stream response pe Accept header / fallback redirect; `case_deadline_add /case/{caseId}/deadline/add` cu form `AddDeadlineType` → `createHearingDeadline()` (JUDECATA, MEDIUM priority); `case_deadline_ruling_date /case/{caseId}/ruling-communication-date` cu form `RulingCommunicationDateType` → set câmp + AuditLog `CATEGORY_DEADLINE_CREATED` + trigger automat `createAppealDeadline()` dacă status ∈ {ORDONANTA_EMISA, IN_ANULARE}). Voter nou `CaseVoter::DEADLINE_MANAGE` ownership-only (NU restricție status — termenele se gestionează în orice stadiu dosar, inclusiv DEFINITIVA/INCHIS_SUCCES). 2 FormType-uri (`AddDeadlineType` cu `deadlineDate ≥ today` + description max 255; `RulingCommunicationDateType` cu `rulingCommunicationDate ≤ today`). Patch `optimistic-action_controller.js` cu CSRF token via Stimulus value + `Turbo.renderStreamMessage()` pe response. 4 templates noi (`_modal_add_deadline`, `_modal_set_ruling_communication_date` cu hint juridic idempotency + warning curier privat referit prin alert_set_ruling_date, `_ruling_communication_date_alert` amber persistent pentru status ORDONANTA/IN_ANULARE fără ruling date, `_deadline_card_turbo_stream` wrapper Turbo Stream replace). Modificat `_deadline_card.html.twig` (drop aria-disabled checkbox, wire optimistic-action button + ID wrapper `deadline-card-{id}`) + `_deadlines_header.html.twig` (drop aria-disabled buton „+ Adaugă custom" + wire `data-hs-overlay`) + `overview.html.twig` (include alert + 2 modale). i18n RO+EN paritate ~25 chei noi `case_overview.deadlines.modal_*` + `alert_set_ruling_date_*` + `flash_*` + `stimulus.deadline.*` + idempotency_note. **Closes W1 legal Pas 4.2**: input rulingCommunicationDate + trigger explicit `createAppealDeadline()` activează CERERE_IN_ANULARE 10 zile (CPC art. 1024). 14 teste WebTestCase: mark complete happy + Turbo Stream + idempotent + CSRF + 403; add JUDECATA happy + past date reject + 403 + CSRF; setRuling happy + skip pre-ordonanta + idempotent + 403 + CSRF; plus 2 teste `CaseVoterTest` pentru `DEADLINE_MANAGE` (owner can in 4 statusuri inclusiv DEFINITIVA + non-owner denied). Suite **826/29/2** vs baseline 810/29/2 = **+16 teste, zero regresii**. Schema unchanged. Reviews legal LEGAL-CLEAN (11 NOTES) + code NEEDS-FIX (2 BLOCKERS + 6 W) → all addresate în same commit. **Faza 4 COMPLETĂ end-to-end vizibil**: avocat creează dosar → PRESCRIPTIE automat → generează somația → RASPUNS_SOMATIE automat → click checkbox → completed instant cu Turbo Stream → add JUDECATA via modal → set rulingCommunicationDate → CERERE_IN_ANULARE automat. Detalii: `~/.claude/projects/-Users-alexc-Downloads-myprojects-symfony-mvp/memory/project_lexrecovery_pas_4_3.md`.
...
### PASUL 5.2 | `PaymentOrderRequestGeneratorService` + opis + ZIP | 1 zi | 50% reutilizare ✅ DONE 2026-05-16 (`af6e247`)

**Rezultat**: ✅ DONE 2026-05-16 (commit `af6e247`). **Faza 5 COMPLETĂ end-to-end**. Livrat: `App\Service\Document\PaymentOrderRequestGeneratorService` + `OpisGeneratorService` (extind `AbstractPdfGenerator` cu type CERERE_OP / OPIS) + `CaseFilesPackager` (ZipArchive cu structură numerotată `01_cerere_ordonanta_plata.pdf` + `02_opis_documente.pdf` + `03_somatie_de_plata.pdf` + `dovada_comunicare/` + `anexe/`) + `CasePaymentOrderController` (POST /case/{id}/payment-order/generate cu CSRF + voter TRANSITION + status guard SOMATIE_TRIMISA + court guard + idempotency + rate limiter dedicat `payment_order_generation 20/h` + wrapInTransaction: 2 PDF-uri + workflow `depune_cerere` + AuditLog `CATEGORY_PAYMENT_ORDER_GENERATED`; GET /case/{id}/zip-package/download cu voter VIEW + guard 3 docs prezente + BinaryFileResponse). 2 templates PDF cu conținut juridic complet CPC art. 1014-1024 + 1016: `payment_order_request.html.twig` (antet instanță + multi-debitor loop + tabel sume cu taxa timbru HARDCODED RON per OUG 80/2013 art. 6 alin. 2 + temei + anexe + semnătură); `document_index.html.twig` (tabel numerotat sortat). UI wire: drop aria-disabled pe 4 elemente (modal confirm + zip card + recommended actions + documents row 2/3); modal_generate_op convertit la form POST cu CSRF; zip_package_card conditional render (modal trigger când fără cerere OP / link direct GET când există); _documents_generated_list cu 3 stări per slot (download/CTA/missing). **Opțiunea A — split „Documente generate" vs „Documente sursă" prin enum** `DocumentType::isAutoGenerated()` (single source of truth: SOMATIE/CERERE_OP/OPIS) + `DocumentRepository::findGeneratedByCase()` + `findSourceByCase()` (utilități API/jobs viitoare). **Optimizare 1-query**: drop `findByCase` din controller + `hasCommunicationProof` folosește `$case->getDocuments()->exists()` (EXTRA_LAZY hidratare unică reutilizată în toate 6+ partials). Reviews post-implementare addresate same commit: B1 legal (taxa timbru moneda), W1 (multi-debitor loop), W5 (exclude CERERE_OP din opis), W3 (rate limiter dedicat), W4 (drop ePPK din footer), em-dash fix. 27 teste verzi (5 PaymentOrder + 5 Opis + 4 Packager + 9 Controller cu AuditLog assert + 4 Repository + extension testIsAutoGenerated). Suite globală **853/29/2** vs pre-Pas 826/29/2 = **+27 teste, zero regresii**. Schema unchanged (zero migrări). i18n RO+EN paritate ~40 chei noi `pdf.payment_order.*` + `pdf.document_index.*` + flash messages + rate limit. **Deferred post-MVP**: M2 cheltuieli judecată (onorariu avocat + alte costuri pe LegalCase), M4 împuternicire avocațială obligatorie pre-depunere, edit cerere OP după generare, cron cleanup ZIP. Detalii: `~/.claude/projects/-Users-alexc-Downloads-myprojects-symfony-mvp/memory/project_lexrecovery_pas_5_2.md`.
...
**Rezultat** (DONE 2026-05-23):
- **Politică CONSERVATOARE auto-tranziții** (confirmată cu utilizatorul, validată avocat-senior): `MonitoringEventApplier` aplică AUTO doar tranzițiile pe date structurate / cu efect protectiv — `HEARING_SCHEDULED`→`fixeaza_termen` + termen `JUDECATA`, `APPEAL_FILED`→`formuleaza_cerere_anulare`. Evenimentele bazate pe interpretarea textului liber al soluției (`HEARING_COMPLETED`/`RULING_ISSUED`) rămân **propuneri** (audit `portal_transition_proposed` + sugestie advisory), avocatul aplică manual din UI (Pas 7.2). Motiv: o `emite_ordonanta` greșită declanșează termenul critic de 10 zile (CPC art. 1024) pe date eronate.
- **Fix fundații legacy**: `CaseMonitoringService` interoghează portalul cu `getCourtCaseNumber()` (nr. dosar instanță) NU `getCaseNumber()` (nr. intern); `MonitorCourtCasesCommand` + `findActiveForMonitoring()` folosesc `CaseStatus::isActiveOnPortal()` (nu statusuri string legacy); `findMonitorableCases()` deprecated (eliminare la 6.2).
- **UI wired**: `CasePortalController` (POST activate cu `inregistreaza_dosar` din CERERE_DEPUSA + check-now cu rate limiter `portal_check` 10/h); `PortalActivateType` (validare format Ecris); template-uri `_portal_config`/`_portal_timeline`/`_portal_sync_status` (badge din `portalMonitoringActive`, form-uri reale, eliminat shell `aria-disabled`).
- **Entitate**: `LegalCase.portalMonitoringActive` (bool DEFAULT 0) + migrare `Version20260523163245`.
- **Subscriber observațional** (`PortalMonitoringSubscriber` postUpdate logger-only, evită flush-in-lifecycle); audit durabil în controller.
- **Teste** (Pas 🔴, 3 niveluri): 50 teste verzi — unit (`PortalMonitoringSubscriberTest`), integration (`MonitoringEventApplierTest`, `CaseMonitoringServiceTest` refactorizat, `CasePortalControllerTest`, `LegalCaseRepositoryTest` reparat+extins), cascade (`PortalMonitoringCascadeTest`), command (`MonitorCourtCasesCommandTest` extins). Suită completă 878 teste, 12 erori + 2 eșecuri (IDENTIC baseline, toate din teste legacy `setStatus('string')`).
- **Reviews**: legal `LEGAL-NEEDS-FIX`→corectat (i18n „contestații"→„cerere în anulare" + disclaimer aliniat la comportamentul real, audit `courtCaseNumber`); code `NEEDS-FIX`→corectat (BLOCKER `.env EXTRACTION_FORCE_STRATEGY` resetat, migrare `DEFAULT 0`, `array_values`, rate limiter `portal_check`, cleanup tearDown). Deferred: W2 nextWorkingDay pe data ședinței (moștenit din `DeadlineService` Pas 4.1), W4 `EXECUTARE` în `isActiveOnPortal` (scope MVP).
...
- **Alerte termene**: `DeadlineAlertService::processAlerts($now)` (Deadline/) emite la praguri 7/3/1 zile + expirat (cel mai strâns prag nesetat, robust la cron ratat), setează flag-uri dedup (`alertSent7/3/1/Expired`) și dispecerizează `DeadlineAlertEvent` (poartă `LegalDeadline` întreg → `getType()` pentru a diferenția PRESCRIPTIE neprorogabilă în 6.3). NU trimite email direct (granița 6.2↔6.3). `DeadlineAlertReport` DTO. Nou `LegalDeadlineRepository::findIncomplete()`.
- **Auto-marcare definitivă (C5)**: `CaseAutoFinalizer::process($now)` iterează `ORDONANTA_EMISA` (nou `LegalCaseRepository::findByStatus()`); `deadline = nextWorkingDay(rulingCommunicationDate + 10z)` (CPC art. 1024 de la COMUNICARE + prorogare CPC art. 181 alin. 2) `+ buffer` (`app.legal_deadlines.auto_final_buffer_days=1`); finalizează doar dacă `now >= deadline+buffer` și `workflow->can(marcheaza_definitiva)`; `rulingCommunicationDate IS NULL` → `MissingCommunicationDateEvent` (NU finaliza); IN_ANULARE exclus prin filtru de status. Audit `CATEGORY_AUTO_FINALIZED`. `AutoFinalizeReport` DTO.
- **`app:check-deadlines`** (cron 07:00, `ClockInterface`, `--format=text|json`): rulează ambele servicii.
- **`app:portal-check-all`** (rename din `MonitorCourtCasesCommand`, cron 08:00): dispecerizează `CheckCasePortalMessage` per dosar cu `DelayStamp` eșalonat (2-5s, rate-limiting), procesat async de worker cu retry (`retry_strategy` max 3 ×2). `monitorCase()` acum **propagă** `PortalJustException` (apelanții sincroni o prind deja). `findMonitorableCases()` eliminat.
- **Cron NU pornește din `docker-entrypoint.sh`** (comentariu adăugat); programare în Coolify la Pas 9.1.
- **Teste** (73 verzi): `DeadlineAlertServiceTest` (praguri/dedup), `CaseAutoFinalizerTest` (4 teste C5: weekend 06-04→06-14 duminică, holiday 12-15→12-25 Crăciun, missing date, IN_ANULARE skip), `CheckCasePortalMessageHandlerTest` (invoke/skip-null/skip-deleted/rethrow), `PortalCheckAllCommandTest` (InMemoryTransport dispatch), `CheckDeadlinesCommandTest` (smoke text+json). Suită completă 891, 12 erori/2 eșecuri IDENTIC baseline.
- **Reviews**: legal `LEGAL-CLEAN` (algoritm C5 confirmat corect, 3 W minore documentate); code `NEEDS-FIX`→corectat (BLOCKERS strict_types ×4; W rename `TermenAlertEvent`→`DeadlineAlertEvent`, constante praguri, test deleted-case, comentarii C5 +10/recalcul).
...
### PASUL 6.3 | `EmailNotificationSubscriber` + Mercure push + templates email | 0.75 zi | 30% reutilizare ✅ DONE 2026-05-24

**Rezultat** (DONE 2026-05-24):
- **`EmailNotificationSubscriber`** (`#[AsEventListener]`, 8 listeneri: 5 tranziții workflow `entered.{SOMATIE_TRIMISA,CERERE_DEPUSA,ORDONANTA_EMISA,DEFINITIVA,RESPINSA}` + `DeadlineAlertEvent` + `MissingCommunicationDateEvent` + `PortalEventDetectedEvent`). Strat subțire de mapare, fără I/O propriu; diferențiază `PRESCRIPTIE` (drept material neprorogabil NCC art. 2517) de procedural (CERERE_IN_ANULARE, 10z de la comunicare CPC art. 1024).
- **`NotificationDispatcher`** (`NotificationDispatcherInterface` + DTO `NotificationDispatch`): fan-out pe 3 canale **fault-isolated** (email + in-app `Notification` + Mercure toast), **headless-safe** (fără Security/RequestStack, rulează în cron + worker). NU reutilizează `ToastService` (acela depinde de request).
...
>    - `workflow.legal_case.entered.ordonanta_emisa` → email "Ordonanță emisă! Termen cerere în anulare 10 zile de la comunicare (CPC art. 1024)".
>    - `workflow.legal_case.entered.definitiva` → email "Titlu executoriu obținut".
>    - `workflow.legal_case.entered.respinsa` → email "Soluție nefavorabilă (cerere de OP respinsă sau cerere în anulare admisă)".
>    - Eveniment custom `App\Event\DeadlineAlertEvent` (dispatch din `DeadlineAlertService`, livrat la Pas 6.2; poartă `LegalDeadline` întreg → `getType()` pentru a diferenția PRESCRIPTIE neprorogabilă) → email "Termen X — N zile rămase".
>    - Eveniment custom `App\Event\MissingCommunicationDateEvent` (dispatch din `CaseAutoFinalizer`, livrat la Pas 6.2) → email "Completează data comunicării ordonanței".
...
>    - La `DeadlineAlertEvent` → publish pe topic `user/{userId}/deadline-alert` (toast urgent).
...
### PASUL 7.1.1 | Listă dosare filtrabilă (pagină distinctă `/cases`) | 0.75 zi | 70% reutilizare ✅ DONE 2026-05-24
...
**Rezultat**: livrat 2026-05-24. Pagina `/cases` (`app_cases`, `CasesController`, `IS_AUTHENTICATED_FULLY`) cu `<twig:DataTable key="cases" />`. `LegalCaseTableDefinition` (auto-înregistrat prin tag): coloane nr. dosar / instanță / sumă / status (`status_badge`) / dată; `createScopedQueryBuilder` scoped pe `t.user` + `deletedAt IS NULL` + `leftJoin court` (anti N+1); `serializeRow` → status `{value,label,color}`, link via `UrlGeneratorInterface` (`case_overview`). **Fundația de filtre decuplate de coloane**: `Filter` DTO readonly (`key/labelKey/type/field/searchFields/options/multiple/placeholderKey`) + `TableDefinitionInterface::getFilters(): Filter[]` (Notificări → `[]`, rămâne pe header-filters); `TableDataService` whitelist = coloane (`filterable`) ∪ `getFilters()`, `applyCriteria` cu tipuri `text` (LIKE escaped) / `bool` / `enum` (`IN`, multi-valoare) / `search` (OR LIKE peste `searchFields`). Toolbar generic server-rendered în `DataTable.html.twig` (search input + enum multi-select); controller Stimulus montează tom-select (chips, `remove_button`) pe `tableBuilt`, debounce 300ms pe search, `setFilter()` / „Șterge filtre". Filtre dosare: status (enum multi-select) + search (`caseNumber` + `courtCaseNumber`). `STATUS_PALETTE` extins la 14 tokenuri (acoperă `CaseStatus::color()`). Navigare wired de la placeholder: „Dosare"/„Vezi toate" + breadcrumb-uri overview/wizard → `app_cases`. i18n RO+EN în lockstep. **10 teste noi** (6 `LegalCaseTableDefinitionTest` prin `TableDataService`: scoping/enum IN multi/search OR/sort numeric DECIMAL/serialize/paginare + 3 `CasesControllerTest` WebTestCase: randare + API status + API search + scoping + 1 `TabulatorRequestParserTest` multi-valoare). Suită **943/9/2** (zero regresii vs baseline 933/9/2). Reviews: **legal LEGAL-CLEAN** (GDPR payload minimal, tenant isolation SQL-level, soft-delete OK), **code COMMIT-READY** după 2 W fix-uite (link hardcodat → `UrlGeneratorInterface`; import CSS tom-select lipsă) + docblock fără referință la ordinea pașilor; W „sort lexicografic amount" INVALID (coloana e `decimal(12,2)` → sort numeric corect). Commit: `feat(dosare): filterable cases list on the Tabulator table foundation`.
