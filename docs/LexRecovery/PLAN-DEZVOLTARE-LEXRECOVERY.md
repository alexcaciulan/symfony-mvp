...
| 3.0 | Wizard | Step 0 wizard "Documente sursă": upload + procesare async (Mercure-first cu polling fallback) + side-card agregat „Date detectate" + `PrefillFromExtractionService` (mutat din 3.1 per analiză 2026-05-11) + migrare `Document.legalCase` NULLABLE | 1.5z | 2.5.8, 2.6, 2.7 | 0% | ✅ DONE 2026-05-11 (`8a54667` base + `7630abc` full creditor/debtor coverage + `012293d` coverage-weighted globalConfidence + dev knobs) — cascadă funcțională end-to-end pe documente reale; **REVIZIE retroactivă Pas 2.5.2** formula globalConfidence migrată la coverage-weighted (`CoverageConfidenceCalculator`) |
