...
- **🔴 Aplicabilitate per sub-pas (Pas 2.5+)**: orice strategie/parser/integrator nou (`OcrTextExtractionStrategy` la 2.5.7, `AiVisionExtractionStrategy` la 2.5.8, `AnthropicApiClient` la 2.5.6, `TesseractOcrService` la 2.5.5, dar și viitoare `PortalEventDetector` extensions, `AnafLookupService` schimbări, etc.) **trebuie să livreze toate 3 niveluri** la marcarea DONE. Reviewer-ul code-reviewer va respinge sub-pași unde lipsesc integration tests pe fixturi realiste.
